...
import { ChevronDown, ChevronUp, RefreshCw, Trash2, Clock } from 'lucide-react';


function RestockHistory({ refreshTrigger }) {
  ...
  const handleUndo = async (requestId, productName, requestedBy) => {
    ...
    if (!confirm(`Delete restock request for ${productName} by ${requestedBy}?`)) return;
      ...
      {isExpanded && (
        <div className="border-t px-6 pb-6">
          <div className="flex items-center justify-between py-4">
            <h3 className="font-medium text-gray-800">Recent Restock Requests</h3>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b bg-gray-50">
                  ...
                  <th className="text-left p-3 font-medium text-gray-700">Requested By</th>
              ...
              <tbody>
                {history.map((request) => (
                  <tr key={request.request_id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="p-3">
                      <button
                        onClick={() => handleUndo(request.request_id, request.product_name, request.requested_by_name)}
